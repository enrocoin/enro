...
Core tests take longer than any other Enro tests, due to the high amount of computational work involved in validating core components.
...
To run only Enro's core tests (after building):
...
To run only Enro's crypto tests (after building):
...
```
enrod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `enro-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Enro's hash tests (after building):
...
To run only Enro's performance tests (after building):
...
To run only Enro's unit tests (after building):
